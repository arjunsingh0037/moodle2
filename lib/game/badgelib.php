<?php
	...
	function process_system_badges($userid)
	{
		...
		badge_cobc_2016($userid);
		badge_security_2016($userid);
		badge_L2L($userid);
...
function badge_Star_completed($userid) 
	...
	function badge_cobc_2016($userid) 
	{
		$cobc=get_record_sql("SELECT gg.finalgrade as grade
		FROM mdl_course AS c JOIN mdl_context AS ctx
		ON c.id = ctx.instanceid and c.visible=1 JOIN mdl_role_assignments AS ra
		...
		ON gg.userid = u.id JOIN mdl_grade_items AS gi
		ON gi.id = gg.itemid and gg.finalgrade >= gi.grademax
		WHERE  gi.courseid = c.id AND gi.iteminstance = '3409' and u.id=$userid");
		if($cobc->grade>0)
		{ 
			return badge_issue('KNOW THE CODE BADGE 2016',$userid);		
		}
		else
		{return false;}
	}
	...
	function badge_security_2016($userid) 
	{
		$security=get_record_sql("SELECT gg.finalgrade as grade
		FROM mdl_course AS c JOIN mdl_context AS ctx
		ON c.id = ctx.instanceid and c.visible=1 JOIN mdl_role_assignments AS ra
		...
		ON gg.userid = u.id JOIN mdl_grade_items AS gi
		ON gi.id = gg.itemid and gg.finalgrade >= gi.grademax
		WHERE  gi.courseid = c.id AND gi.iteminstance = '3404' and u.id=$userid");
		if($security->grade>0)
		{ 
			return badge_issue('Global Information Security 2016',$userid);		
		}
		else
		{return false;}
	...
	/* Badge for L2L*/
	function badge_L2L($userid) 
	{
		$L2L=get_record_sql("SELECT gg.finalgrade as grade
		FROM mdl_course AS c JOIN mdl_context AS ctx
		ON c.id = ctx.instanceid JOIN mdl_role_assignments AS ra
		ON ra.contextid = ctx.id JOIN mdl_user AS u
		ON u.id = ra.userid JOIN mdl_grade_grades AS gg
		ON gg.userid = u.id JOIN mdl_grade_items AS gi
		ON gi.id = gg.itemid and gg.finalgrade >= gi.grademax
		WHERE  gi.courseid = c.id AND gi.itemtype = 'course' and u.id=$userid and c.id =8883");
		if($L2L->grade>0)
		{ 
			return badge_issue('Leader to Leader Program Badge',$userid);		
		}
		else
		{
		return false;
