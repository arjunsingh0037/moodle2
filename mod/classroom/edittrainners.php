...
if ($frm = data_submitted()) {
    ...
    if ($add and !empty($frm->addselect) and confirm_sesskey()) {
		...
		//	role_assign(4, $user, 0, $context->id, 0, 0, 0)

                if (! role_assign(4, $user->id, 0, $context->id, 0, 0, 0)) {
           ...
           //******* cheching user session code deleted : Roy Philip
            ...
            if (!classroom_trainner_signup($session, $classroom, $course, '', MDL_F2F_ICAL,
                ...
                $errors[] = get_string('error:editattendees', 'classroom', fullname($erruser));
            }
        }
    }
    // Remove button
        ...
        foreach ($frm->removeselect as $removeuser) {
            ...
            if (classroom_trainner_cancel($session, $removeuser)) {
...
$navlinks[] = array('name' => get_string('Trainners', 'classroom'), 'link' => "trainners.php?.php?s=$session->id", 'type' => 'activityinstance');
$navlinks[] = array('name' => get_string('addremovetrainners', 'classroom'), 'link' => '', 'type' => 'title');
$navigation = build_navigation($navlinks);
print_header_simple($pagetitle, '', $navigation, '', '', true,
                    update_module_button($cm->id, $course->id, $strclassroom), navmenu($course, $cm));

print_heading(get_string('addremovetrainners', 'classroom'), 'center');
///*********** Roy Philip:Code Modified to acess trainners available not available info **************
/// Get the list of currently signed-up users
$existingusers = classroom_get_trainners($session->id);
...
$availableusers = get_recordset_sql('SELECT id,username, firstname, lastname, email
                                      ...
                                      WHERE '.$select.'
                                        AND id NOT IN
                                          (
                                            SELECT u.id
                                              FROM '.$CFG->prefix.'classroom_trainners s
...
$url = $CFG->wwwroot.'/mod/classroom/trainners.php?s='.$session->id;
